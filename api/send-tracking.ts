// POST /api/send-tracking — Vercel Edge Function
//
...
// Same branded shell used for order confirmation emails (api/webhook.ts) so
// all outgoing customer emails look consistent.
...
export default async function handler(request: Request): Promise<Response> {
  ...
  const adminKey = process.env.ADMIN_SEND_KEY
  if (!adminKey) {
  ...
  if (str(payload.adminKey) !== adminKey) {
