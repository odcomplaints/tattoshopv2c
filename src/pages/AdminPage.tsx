...
function SortSection({ items }: { items: ShopItem[] }) {
  ...
  function generateFeaturedOrderTs(): string {
    const lines: string[] = []
    lines.push(`// Manually curated shop display order ("Featured" sort on /shop).`)
    lines.push(`//`)
    lines.push(`// This file is generated by the admin panel's Sortierung tool: drag items`)
    lines.push(`// (or use the arrows / position field) into the order you want, click`)
    lines.push(`// "Reihenfolge kopieren", then paste the copied code here, replacing`)
    lines.push(`// everything below, and save. No other steps needed — the shop page reads`)
    lines.push(`// straight from this file.`)
    lines.push(`//`)
    lines.push(`// Items not listed here (e.g. newly added products) fall back to appearing`)
    lines.push(`// after all listed items, in their natural array order.`)
    lines.push(``)
    lines.push(`export const FEATURED_ORDER: string[] = [`)
    for (const item of order) {
      lines.push(`  '${escapeSingle(item.id)}',`)
    }
    lines.push(`]`)
    lines.push(``)
    return lines.join('\n')
  ...
  async function copyOrder() {
    const text = generateFeaturedOrderTs()
    try {
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-neutral-400">
            ...
            Platznummer in die gewünschte Reihenfolge bringen, dann auf „Reihenfolge kopieren" klicken und den
            Code 1:1 in <code className="text-neutral-300">src/data/featuredOrder.ts</code> einfügen (alles
            ersetzen) und speichern. Mehr ist nicht nötig.
          </p>
        </div>
        <button
          type="button"
