...
const getFontValue = (value: number | string) => (typeof value === 'number' ? `${value}px` : value)
...
const syncUniforms = (program: Program, props: WarpTextRuntimeProps) => {
  const uniforms = program.uniforms as Record<string, { value: number }>
...
const WarpText = ({
  ...
  const propsRef = useRef<WarpTextRuntimeProps>({
    text,
    color,
    fontSize,
    fontWeight,
  ...
  const contextRef = useRef<{ program: Program; rasterize: () => Promise<void> } | null>(null)
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      ;(program.uniforms.uPointer.value as Float32Array)[0] = pointer.x
      ;(program.uniforms.uPointer.value as Float32Array)[1] = pointer.y
