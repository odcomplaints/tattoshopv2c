// InfiniteMenu — adapted from react-bits (DavidHDev/react-bits, MIT +
// Commons Clause license) for this project's TypeScript setup. Renders a
// set of images on the inside faces of a subdivided icosahedron (WebGL2)
// that the user can drag to spin; the item facing the camera "snaps" into
// focus and shows its title.
//
// See src/components/InfiniteMenu.css for the accompanying styles.

import { useEffect, useRef, useState } from 'react'
import { mat4, quat, vec2, vec3 } from 'gl-matrix'
import './InfiniteMenu.css'

export type InfiniteMenuItem = { image: string; title?: string; description?: string; link?: string }
...
function makeBuffer(gl: WebGL2RenderingContext, sizeOrData: ArrayBufferLike | ArrayBufferView, usage: number) {
  const buf = gl.createBuffer()
  gl.bindBuffer(gl.ARRAY_BUFFER, buf)
  gl.bufferData(gl.ARRAY_BUFFER, sizeOrData as never, usage)
  gl.bindBuffer(gl.ARRAY_BUFFER, null)
  return buf as WebGLBuffer
}
...
class ArcballControl {
  ...
  update(deltaTime: number, targetFrameDuration = 16) {
    ...
    this.#rotationVelocity += (rv - this.#rotationVelocity) * RV_INTENSITY
    this.rotationVelocity = this.#rotationVelocity / timeScale
...
class InfiniteGridMenu {
  ...
  discBuffers!: { vertices: Float32Array; uvs: Float32Array; indices: Uint16Array }
  ...
  constructor(
    ...
    onActiveItemChange: (index: number) => void,
    onMovementChange: (moving: boolean) => void,
    onInit?: ((sk: InfiniteGridMenu) => void) | null,
    scale = 1.0,
    subdivisions = 1,
  ) {
    this.canvas = canvas
    ...
    // More subdivisions => more, smaller discs so they still tile the sphere nicely.
    this.discScale = 0.25 / Math.max(1, subdivisions)
  ...
  #init(onInit?: ((sk: InfiniteGridMenu) => void) | null) {
    this.gl = this.canvas.getContext('webgl2', { antialias: true, alpha: true }) as WebGL2RenderingContext
    const gl = this.gl
    if (!gl) {
  ...
  #render() {
    ...
    gl.uniformMatrix4fv(this.discLocations.uWorldMatrix as WebGLUniformLocation, false, this.worldMatrix as Float32Array)
    gl.uniformMatrix4fv(this.discLocations.uViewMatrix as WebGLUniformLocation, false, this.camera.matrices.view as Float32Array)
    gl.uniformMatrix4fv(
...
type InfiniteMenuProps = {
  items?: InfiniteMenuItem[]
  scale?: number
  /** Icosahedron subdivision level — higher = more, smaller faces (more "globe"-like). */
  subdivisions?: number
  backgroundColor?: string
  showLabels?: boolean
  /** Text shown inside the action button. Defaults to an arrow icon (↗). */
  actionLabel?: string
  /** Background color of the action button. Defaults to the theme's dark navy. */
  actionButtonColor?: string
  onItemOpen?: (item: InfiniteMenuItem) => void
}
...
export default function InfiniteMenu({
  ...
  return (
    ...
    >
      ...
      {activeItem && ((showLabels && (activeItem.title || activeItem.description)) || onItemOpen) && (
        <>
          {showLabels && activeItem.title && (
            <h2 className={`face-title ${isMoving ? 'inactive' : 'active'}`}>{activeItem.title}</h2>
          )}
          {showLabels && activeItem.description && (
